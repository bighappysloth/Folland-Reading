...
\end{definition}
\begin{wts}
    % If $f$ is differentiable at $x$, then the $\lambda$ in \cref{eq:differentiability-def} is unique. We write $f'(x) = Df(x) = \lambda$ as in \cref{eq:differentiability-def}. Furthermore, if $f'(x)$ and $g'(x)$ exist, then $(f+g)'(x) = f'(x) + g'(x)$ as linear maps, similar for scalar multiplication.
