...
\begin{itemize}
    ...
    \item Creating a $k$-list: $(\UL{v}[k]) = (v_1,\ldots, v_k)\in \prod E_{j\leq k}$
    ...
    \]
    \item Empty list is iterated $0$ times: 
    \[
        (\UL{v}[0], a,b,c) = (a,b,c)
    \]
    ...
    \]
    Of course, if $i=1$, then the above expression reads $(f(v_1), v_2\ldots, v_k)$ by the previous bullet point.
    \item Skipping an index: 
    \[
        (\UL{v}[i-1],\UL{v}[i+][k-i]) = (v_{1},\ldots, v_{i-1},v_{i+1},\ldots, v_{k})
    \]
    for $i = \underline{k}$.
    \item In any list using this 'underline' notation, we can find the size of a list by summing over all the underlined terms.
...
\end{remark}
If $E_1 = \cdots = E_k = E$, we write $L(\UL{E}[k]; F) = L(E^k, F) = L^k(E,F)$.
