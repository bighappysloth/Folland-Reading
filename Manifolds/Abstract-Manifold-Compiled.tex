...
\begin{enumerate}
    ...
    \item The definition of smoothness between open subsets of Banach spaces (see \cref{def:cp-banach-space}) is a purely local one. And let us recall: \textbf{every chart domain $U$ in a manifold $X$ corresponds to an open subset $\hat{U}\subseteq E$ in the model space}. \\
    
    The necessity that $f(U)$ must be contained in a single chart domain of $Y$ is a relic of the original definition (see \cref{rmk:cp-banach-space-mapping-image-contained} as well). 
\end{enumerate}
%
