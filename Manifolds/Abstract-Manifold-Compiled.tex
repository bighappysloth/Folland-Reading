...
\begin{tikzcd}
	...
	\arrow["df"{description}, from=1-1, to=3-1]
\end{tikzcd}
...
\end{figure}
    ...
    Let us use $\iota_{S}: S\to X$ to represent the inclusion map and consider a fixed point $p\in S$. It is always possible to identify a slice chart $(U,\varphi)$ within $X$ that contains $p =\iota_S(p)$ in its domain. By definition of the atlas on $S$, this induces a 'truncated' chart $(U^s,\varphi^s)$. \\
...
\begin{definition}[Section of a vector bundle]\label{def:section-of-VB}
    Let $W$ be a bundle over a manifold $X$. A \emph{section} of $W$ is a morphism $\sigma\in\Mor(X,W)$ such that the diagram in \cref{fig:VB-section-diagram}  commutes, or $\pi\sigma = \id{X}$. A \emph{local section} of $W$ is a morphism $\sigma: U\to W$ where $U\osub X$ is viewed as a submanifold and $\pi\sigma = \id{U}$.\\
...
\begin{remark}[VB coordinates]\label{rmk:VB-coordinates}
    Let $X$ and $W$ be as in \cref{def:section-of-VB}. If $\sigma$ is a section on $W$, $p\in X$. Using a pair of VB charts, $(U)\in X$ and $(\wig{U})\in W$, we define the \emph{VB coordinates of $\sigma$} \begin{equation}\label{eq:VB-coordinates}
    ...
    \]
    We have achieved our goal of constructing morphisms whose coordinate representation locally resembles a graph: $\hat{\sigma}:\hat{U}\to\hat{U}\times F$ and because of this, we identify $\hat{\sigma}(\hat{p}) = (\hat{p},v)\in \hat{U}\times F$ with $v\in F$.
\end{remark}
...
% https://q.uiver.app/#q=
\begin{subfigure}[b]{.5\textwidth}
    \centering
    \begin{tikzcd}
	{\wig{U}} && {U\times F} \\
    ...
    \end{tikzcd}    
    \caption{Coordinates of a VB section}
