...
\end{equation}
However, $\varphi$ is a $C^p$ isomorphism onto $\hat{U}$, and it is not hard to see that the restriction we must place on $E_1$ is that \textbf{it must split in $E$}. Let $\alpha$ be a $C^p$ isomorphism between $E$ and $E_1\times E_2$. \Cref{eq:motivation-submanifold} becomes
\begin{equation}\label{eq:motivation-submanifold-modified}
    \alpha\varphi(S\cap U) = V_1\times a_2\qqtext{where} V_1\osub E_1 \text{ and }a_2\in E_2
\end{equation}
Identifying $\hat{U}$ with $\alpha(\hat{U})$, we arrive at the following defintion.
