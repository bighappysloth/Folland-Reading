...
\topheader{Embedded submanifolds}
...
\begin{equation}
    \hat{F}_{jk}:\varphi_{jk}(U_{jk})\to \psi_{jk}(V_{jk}),\quad \hat{F}_{jk}\defined \psi_{jk}\circ F_{jk}\circ \varphi_{jk}
\end{equation}
\end{step}
\begin{proof}
...
\begin{step}[Change of Coordinates]
    ...
    a\in V_a\subseteq\cl{V}_a\subseteq B\subseteq U
    \end{equation}
\end{step}
\begin{proof}
If $(x^1,\ldots,x^m)$ are the standard coordinates of $\realm$, the map 
    ...
    \[
        \operatorname{Jacobian}(T)(x)=\begin{bmatrix}
            \pdv{F^1}{x^1}(x) & \cdots \\ 0 & Id_{m-1}
        \end{bmatrix}
    \]
    and it is invertible at $a$. The continuity of $\pdv{F^1}{x^1}$ at $a$ gives us an open ball $B$ containing $a$, on which $\pdv{F^1}{x^1}\neq 0$. Let $V_a$ be an open, precompact neighbourhood that hides in this ball as in \Cref{lee-sards-LCH}. \\
...
\begin{step}[Prove {$F(C\setminus C_1)$} has measure zero]\label{lee-sards-case1}
