...
For each $a\in A$, induces disjoint open sets $U_a$, $V_a$ that wrap $a$ and the closed set $B$. We can assume by [this trick]({% post_url 2023-05-11-notes-on-separation-in-topology %}), that $a\notin\cl{V_a}$. Take the collection of all such 

$$
    \Phi = \underbrace{\{V_a\}_{a\in A}}_{\text{covers } A}\cup\underbrace{\{A^c\}}_{\text{covers }A^c}
$$
