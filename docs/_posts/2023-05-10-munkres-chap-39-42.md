...
<div class="lemma-box" markdown=1 name="Properties of local finiteness">
If $$\mathcal{A} =\{E_{\alpha}\}$$ is a family of locally finite subsets of $\xx$, then:
- Any subcollection of  $\mathcal{A}$ is locally finite.
- The collection $$\borel = \{\cl{E}_{\alpha}\}$$ is again locally finite.
- The closure of the union over $$\{E_{\alpha}\}$$ is equal to the closure of the union.
\begin{equation}
\cl{\bigcup E_{\alpha}} =\bigcup \cl{E}_{\alpha}
\label{eq:locally-finite-closure-union}
\end{equation}
</div>
<div class="proof-box" markdown=1 name="">
The first claim follows upon taking the contrapositive. Let $x\in \xx$ st every neighbourhood $U$ of $x$ intersects infinitely many $\cl{E_\alpha}$ from $\borel$. at every $\alpha$ with a non-trivial intersection, $U$ is a neighbourhood of some point $y_\alpha\in \cl{E_\alpha}$.
...
Next, denote the left member of \eqref{eq:locally-finite-closure-union} by $\cl{Y}$, so that $Y$ is the union over all $E_{\alpha}$. Since $\cl{Y}$ is a closed superset of each $E_{\alpha}$, it contains the closures of $E_{\alpha}$, and $\cl{Y}\supseteq \bigcup \cl{E}_{\alpha}$.

If $x$ is not in the right member of \eqcref{eq:locally-finite-closure-union}, there exists an *open* neighbourhood $U$ of $x$ that intersects finitely many $E_{\underline{k}}$. Since $x$ is not an adherent point of all $E_j$, for each of the finite $j = \underline{k}$, there exists an open $U_j$ containing $x$, that 'blocks out' all the points from $E_j$. more precisely:

$$
U_j\cap E_j=\varnothing
$$

Let us agree to write $$V = \bigcap U\cap U_{\underline{k}}$$, then $V$ must intersect none of the points in $E_{\alpha}$. So $x$ cannot be an adherent point of $Y$, and the proof is complete.

</div>

<div class="definition-box" markdown=1 name="Countably locally finite">
A collection $\borel$ is **countably locally finite** if it can be written as the countable union of *locally finite* families.

\begin{equation}
\borel = \bigcup\borel_{n\geq 1}
\label{eq:countably-locally-finite}
\end{equation}

where each $\borel_n$ in \cref{eq:countably-loally-finite} is a locally finite family. Note that no restrictions are placed on the cardinality of the individual $\borel_n$.
</div>

<div class="definition-box" markdown=1 name="Refinement">
Let $\mathcal{A}$ be a collection of subsets. Let $\borel$ be a family of subsets of $\xx$. We say $\borel$ **refines** $\mathcal{A}$, whenever each $B\in\borel$ is a subset of some $A\in\mathcal{A}$.
</div>
