...
Let $Y$ be a vector field on $N$, we define $u^*Y (p) = (du^{-1}_u(p))\biggl(Y_u(p)\biggr)$, or written more explicitly

$$
( u^*Y ) (p) = \biggl((du)^{-1}\circ Y\circ u\biggr)(p)
$$
...
- If $K\in C^\infty(N)$ defines a HVF on $N$ through $-\hat{\eta}^{-1}\circ d: K\mapsto -\hat{\eta}^{-1}(dK)= X_K$. Let $u^*$ and $\hat{\omega}$, $\hat{\eta}$ be defined as above, then the following diagram commutes
