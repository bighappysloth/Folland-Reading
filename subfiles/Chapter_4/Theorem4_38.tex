...
\begin{proof}
    ...
    \begin{equation}\label{chp4:4.38-restriction-intersection}
            (f|K)^{-1}(W) = f^{-1}(W)\cap K    
    \end{equation}
    since we can write $(f|K)(x) = (f\circ \iota_K)(x)$. Where $\iota_K: K\to \xx$ is the inclusion map, which is an embedding. \Cref{chp4:4.38-restriction-intersection} follows immediately. Therefore $f^{-1}(W)\cap K = (f|K)^{-1}(W)$ is closed Rel. $K$, so it is closed Rel. $\xx$. This holds for every compact $K$, so $f^{-1}(W)$ is closed for any closed $W\subseteq\complex$, and $f$ is continuous.
\end{proof}
